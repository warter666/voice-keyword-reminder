...
自动弹出 Windows 通知并播放提示音。比如直播里主播说「签到」,你会立刻收到提醒。（哈哈其实并非主播，就是想着***远程签到***：)）
...
7. **验证**:
   ...
   - 或在电脑上运行 `python test_ntfy.py`(自动读取 config.toml 的主题;也可
     `python test_ntfy.py 主题名` 临时指定),成功即说明网络与主题没问题
